...
def vote(vote: schemas.Vote, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    post_query = db.query(models.Post).filter(models.Post.id == vote.post_id).all()
    if not post_query:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"post {vote.post_id} has not exist in post")
