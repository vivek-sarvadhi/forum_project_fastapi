...
def create_post(title: str = Form(...), content: str = Form(...), forum_id: int = Form(...), files: UploadFile = File(...), db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    # cursor.execute(""" INSERT INTO posts (title, content) VALUES (%s, %s) RETURNING * """, (post.title, post.content))
    # new_post = cursor.fetchone()
    # conn.commit()
    # print(**post.dict())
    # print(post)
    with open("media/topic_file/"+files.filename, "wb") as image:
        shutil.copyfileobj(files.file, image)
    url = str("media/topic_file/"+files.filename)
    new_post = models.Post(owner_id=current_user.id, title=title, content=content, topic_file=url, forum_id=forum_id)
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return new_post
